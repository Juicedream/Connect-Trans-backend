...
import { createAccountHtml, forgotPasswordHtml } from "../htmls/html.js";
...
const forgotPassword = async (req, res) => {
  ...
  await sendMail(email, "FORGOT PASSWORD", forgotPasswordHtml(defaultPassword, fullName, TIME));

  console.log(`Default Password email has been sent to ${email}`);

  return res.status(200).json({
    message: `Default password has been sent to ${email}`,
  });


};

export { createAccount, signIn, signOut, me, forgotPassword };

export const clearAllDefaultPasswords = async () => {
  const now = new Date();
